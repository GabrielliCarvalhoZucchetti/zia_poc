...
const AppInner: React.FC = () => {
  ...
  useEffect(() => {
    localStorage.setItem('luna_community_posts', JSON.stringify(communityPosts));
  }, [communityPosts]);
  ...
  useEffect(() => {
    setCommunityPosts(prev => {
      ...
      resources.forEach(res => {
        if (res.environment === ResourceEnvironment.PRODUCTION) {
          const alreadyHasPost = updatedPosts.some(p => p.resourceId === res.id);
          if (!alreadyHasPost) {
            const seedWord = res.name.toLowerCase().replace(/[^a-z0-9]/g, '-');
            const imageUrl = `https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=800&auto=format&fit=crop&q=80&sig=${seedWord}`;

            const newPost = {
              id: `post-auto-${res.id}`,
              title: `${res.name} em Produção! 🚀`,
              content: `Um novo recurso do tipo **${res.type}** foi publicado com sucesso no ambiente de Produção!\n\n**Descrição:** ${res.description}\n\nVocê já pode abrir e interagir diretamente no Playground de desenvolvimento clicando no link abaixo. Aproveite o máximo da nossa IA!`,
              summary: `O novo recurso "${res.name}" acaba de ser publicado em ambiente de Produção. Experimente no Playground!`,
              imageUrl,
              author: res.creatorName || 'Gabrielli Carvalho',
              date: res.createdAt || new Date().toISOString().split('T')[0],
              likes: ['Alice Castro', 'Gabriel Ricardo', 'Ana Costa'],
              comments: [
                {
                  id: `c-auto-${res.id}-1`,
                  user: 'Alice Castro',
                  content: 'Que incrível! Mal posso esperar para testar essa integração no meu fluxo de trabalho.',
      ...
      if (updatedPosts.length === 0 || !updatedPosts.some(p => p.id === 'post-manual-1')) {
        const manualPost = {
          id: 'post-manual-1',
          title: 'Workshop Especial de Prompt Engineering 🌟',
          content: 'Participe do nosso workshop exclusivo sobre Engenharia de Prompts para alavancar sua produtividade. Vamos cobrir técnicas avançadas como Chain-of-Thought, Few-Shot prompting e como configurar agentes complexos de atendimento e automação. O evento acontecerá de forma online no Teams na próxima quarta-feira às 14h.',
          summary: 'Participe do workshop exclusivo sobre Engenharia de Prompts para alavancar sua produtividade corporativa na próxima quarta-feira.',
          imageUrl: 'https://images.unsplash.com/photo-1531482615713-2afd69097998?w=800&auto=format&fit=crop&q=80',
          author: 'Gabrielli Carvalho',
          date: '2026-06-25',
          likes: ['Alice Castro', 'Ana Costa', 'Rodrigo Faro'],
          comments: [
            {
              id: 'c-manual-1-1',
              user: 'Alice Castro',
              content: 'Mal posso esperar por este evento! Já agendei no meu calendário.',
              timestamp: '2026-06-25 15:30:00'
            },
            {
              id: 'c-manual-1-2',
              user: 'Rodrigo Faro',
              content: 'Excepcional! A Zucchetti sempre inovando.',
              timestamp: '2026-06-25 16:15:00'
            }
          ]
        };
        updatedPosts.push(manualPost);
        changed = true;
      }
  ...
  const visibleResources = useMemo(() => resources.filter(res => 
    res.environment === ResourceEnvironment.PRODUCTION || 
    res.creatorId === user.id || 
    user.role === UserRole.ADMINISTRATOR
  ), [resources, user.id, user.role]);
